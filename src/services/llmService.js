...
function shouldForceAdamArchaicTerm(dater, enforceAdamArchaic = false) {
  if (!enforceAdamArchaic) return false
...
function finalizeDaterDialogueLine(text, dater, { enforceAdamArchaic = false } = {}) {
  const cleaned = stripActionDescriptions(text)?.replace(/\s+/g, ' ')?.trim()
  if (!cleaned) return ''
  if (shouldForceAdamArchaicTerm(dater, enforceAdamArchaic)) {
...
- Do not force archaic wording globally.
- Only use exactly one archaic term when the task explicitly asks for it.
...
const PROFILE_MATCH_STOPWORDS = new Set([
  'about', 'after', 'again', 'being', 'their', 'there', 'these', 'those', 'where',
  'would', 'could', 'should', 'because', 'while', 'with', 'from', 'into', 'just',
  'very', 'really', 'your', 'youre', 'they', 'them', 'this', 'that', 'what', 'when',
  'have', 'has', 'been', 'were', 'ours', 'ourselves', 'self', 'over', 'under',
])
...
function findValueHitCategory(valuesContext, playerAnswer) {
  ...
  if (!answerTokens.size) return null

  const categoryOrder = ['dealbreakers', 'dislikes', 'loves', 'likes']
...
export async function generateSpeedDatingOneLinerBatch({
  ...
  const buildEntrySpecBlock = (entry) => {
    ...
    const speakerArchetype = String(speaker?.archetype || 'Unknown')
    const speakerQuirk = clipPromptText(String(speaker?.quirk || ''), 120) || 'none'
    const speakerTraits = normalizePromptList(speaker?.talkingTraits || [], 'not specified')
    const speakerLikes = normalizePromptList(speaker?.idealPartner || [], 'chemistry, confidence, humor')
    const speakerDealbreakers = normalizePromptList(speaker?.dealbreakers || [], 'disrespect, dishonesty')
    ...
    return [
      `- key: ${entry.key}`,
      `  speaker: ${speakerName} (${speakerArchetype})`,
      `  speaker quirk: ${speakerQuirk}`,
      `  speaker traits: ${speakerTraits}`,
      `  speaker likes: ${speakerLikes}`,
      `  speaker dealbreakers: ${speakerDealbreakers}`,
  ...
  const systemPrompt = `You write punchy speed-dating pickup lines.
Return strict JSON only.
No markdown fences.
Write with high variety across the whole batch, not just one line at a time.`

  const parseLinesFromResponse = (parsed) => {
    const out = new Map()
    ...
    if (parsed.lines && typeof parsed.lines === 'object') {
      Object.entries(parsed.lines).forEach(([key, value]) => {
        out.set(String(key || '').trim(), String(value || '').trim())
      })
      return out
    }
    ...
    entries.forEach((entry) => {
      ...
      if (typeof direct === 'string') out.set(entry.key, direct.trim())
    })
    return out
  }

  const scoreBatchVariety = (linesByKey = {}) => {
    const lines = entries
      .map((entry) => String(linesByKey?.[entry.key] || '').trim())
      .filter(Boolean)
    if (!lines.length) return 0

    const openerSet = new Set(lines.map((line) => extractPickupOpener(line, 4)).filter(Boolean))
    const tailSet = new Set(lines.map((line) => extractPickupTail(line, 4)).filter(Boolean))
    const inviteSet = new Set(lines.map((line) => detectPickupInviteCategory(line)).filter(Boolean))
    const nonStaleCount = lines.filter((line) => !PICKUP_STALE_FRAME_PATTERN.test(line)).length
    return (lines.length * 100) + (openerSet.size * 10) + (tailSet.size * 8) + (inviteSet.size * 12) + (nonStaleCount * 8)
  }

  const sanitizeBatchLine = (entry, rawLine = '', usedOpeners = new Set(), usedTails = new Set()) => {
    const speaker = entry?.speaker || {}
    const target = entry?.target || {}
    const line = String(rawLine || '').trim()
    if (!line) return null

    const finalized = finalizeDaterDialogueLine(line, speaker, { enforceAdamArchaic: entry.targetType === 'player' })
    if (!finalized) return null

    const patterns = buildNamePatterns([speaker?.name, target?.name])
    const scrubbed = scrubForbiddenNamesFromPickupLine(clampOneLinerWords(finalized, 16), patterns)
      .replace(/\s+([,.!?;:])/g, '$1')
      .replace(/\s+/g, ' ')
      .trim()
    if (!scrubbed) return null

    const words = countWords(scrubbed)
    if (words < 6 || words > 16) return null
    if (PICKUP_REPLY_PATTERN.test(scrubbed)) return null
    if (hasClichePickupPhrase(scrubbed)) return null
    if (PICKUP_STALE_FRAME_PATTERN.test(scrubbed)) return null
    if (mentionsForbiddenPickupName(scrubbed, patterns)) return null
    if (!/\byou\b/i.test(scrubbed)) return null

    const opener = extractPickupOpener(scrubbed, 4)
    const tail = extractPickupTail(scrubbed, 4)
    if (opener && usedOpeners.has(opener)) return null
    if (tail && usedTails.has(tail)) return null

    const inviteCategory = detectPickupInviteCategory(scrubbed)
    const hasInvite = PICKUP_SIGNAL_PATTERN.test(scrubbed) || PICKUP_INVITE_PATTERN.test(scrubbed) || FLIRTY_SIGNAL_PATTERN.test(scrubbed)
    if (!hasInvite) return null
    if (!(hasHumorSignal(scrubbed) || PICKUP_COMEDY_PATTERN.test(scrubbed) || /[!?]/.test(scrubbed))) return null

    if (opener) usedOpeners.add(opener)
    if (tail) usedTails.add(tail)
    return { text: scrubbed, inviteCategory }
  }
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    const userPrompt = `Generate one pickup line for EACH key below.
Required keys: ${keyList}

Global rules (across all lines):
- Every line must be clearly different in opener and ending cadence.
- Use at least 3 invite styles across the batch (drink/date/kiss/number/challenge).
- No cliches and no stale templates.
- No names in output text (never mention: ${forbiddenNames || 'none'}).
- No "you've got ___ energy", no "buy you a drink", no "you look like trouble".
- No call-and-response callbacks.

Per-line rules:
- One sentence only
- 7-13 words (hard max 16)
- Directly address "you"
- Funny, cheeky, flirty bar pickup-line tone
- Concrete invite beat
...
${feedback ? `\nFix these prior issues: ${feedback}` : ''}`
    ...
    const parsed = safeJsonObject(response)
    const rawByKey = parseLinesFromResponse(parsed)
    ...
    entries.forEach((entry) => {
      ...
      const sanitized = sanitizeBatchLine(entry, raw, usedOpeners, usedTails)
      linesByKey[entry.key] = sanitized?.text || null
      if (sanitized?.text) {
        validCount += 1
        if (sanitized.inviteCategory) inviteCategories.push(sanitized.inviteCategory)
      } else {
        missing.push(entry.key)
      }
    })

    const inviteCategoryCount = new Set(inviteCategories.filter((c) => c && c !== 'other')).size
    const score = scoreBatchVariety(linesByKey) + (inviteCategoryCount * 25)
    if (score > bestScore) {
      bestScore = score
      bestLinesByKey = linesByKey
    }

    if (validCount === entries.length && inviteCategoryCount >= 3) {
      return linesByKey
    }

    feedback = `missing keys: ${missing.join(', ') || 'none'}; unique invite styles: ${inviteCategoryCount}; make all lines more distinct from each other`
  }

  return bestLinesByKey
}
...
- Picked sender must be the highest score (break ties by funnier line).`
...
Choose now. JSON only.`
  ...
  const pickedId = highest?.senderId && highest.senderId !== resolvedPickedId
    ? highest.senderId
    : (resolvedPickedId || highest.senderId)
