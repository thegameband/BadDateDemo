...
function pickThreeDaters(daters = [], nonce = 0) {
  const pool = Array.isArray(daters) ? [...daters] : []
  if (!pool.length) return []

  const priorityNames = ['Leo', 'Kickflip', 'Adam']
...
function buildSpeedDraftSequence(selectedDaters = []) {
  if (!Array.isArray(selectedDaters) || selectedDaters.length < 3) return []
  const [a, b, c] = selectedDaters
  return [
    { id: 'ai-a-b', type: 'ai_pair', first: a, second: b },
    // Player only targets daters they've already seen receive an LLM line.
    // After ai-a-b, dater A has been targeted by B.
    { id: 'player-a', type: 'player_round', target: a },
    { id: 'ai-b-c', type: 'ai_pair', first: b, second: c },
    // After ai-b-c, dater C has been targeted by B.
    { id: 'player-c', type: 'player_round', target: c },
    { id: 'ai-c-a', type: 'ai_pair', first: c, second: a },
    // Dater B has already been targeted in ai-a-b (and again in ai-b-c).
    { id: 'player-b', type: 'player_round', target: b },
  ]
}
...
function summarizeTraits(list = [], fallback = 'Not specified') {
...
export default function SpeedDateDraftMode({ daters = [], onBack }) {
  ...
  const selectedDaters = useMemo(() => pickThreeDaters(daters, runNonce), [daters, runNonce])
  ...
  const recomputeTargetCardPlacement = useCallback(() => {
    ...
    const upTop = rect.top - estimatedCardHeight - 8
  ...
  if (selectedDaters.length < 3) {
    return (
      <div className="speed-date-draft">
        ...
        <div className="speed-date-fallback">Need at least three daters to run this mode.</div>
      </div>
    )
  }
  ...
  return (
    <div className="speed-date-draft">
      <div className="speed-date-draft-header">
        <button type="button" className="speed-date-back" onClick={handleBackClick}>← Back</button>
        <h2 className="speed-date-title">Speed Date Draft</h2>
        <p className="speed-date-subtitle">Three daters. Fast one-liners. You make the final call.</p>
      </div>
      ...
      {stage === 'intro' && (
        <div className="speed-date-intro">
          ...
          <button type="button" className="speed-date-primary-btn" onClick={handleStart}>
            Start Speed Round
          </button>
        </div>
      )}
              ...
              <label className="speed-date-input-label">
                ...
                >
                  ...
                  >
                    ...
                    <p className="speed-date-target-card-tagline">{currentStep.target.tagline}</p>
                    <p className="speed-date-target-card-description">
                      {summarizeFirstSentence(currentStep.target.description, 200)}
                    </p>
                    <div className="speed-date-target-card-row">
                      <span>Values</span>
                      <p>{summarizeFirstSentence(currentStep.target.values, 160)}</p>
                    </div>
                    <div className="speed-date-target-card-row">
                      <span>Likes</span>
                      <p>{summarizeTraits(currentStep.target.idealPartner)}</p>
                    </div>
                    <div className="speed-date-target-card-row">
                      <span>Avoids</span>
                      <p>{summarizeTraits(currentStep.target.dealbreakers)}</p>
                    </div>
                  </div>
                </span>
              </label>
